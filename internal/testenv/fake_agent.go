// RunFakeAgent implements the agent process double for command entrypoints and re-executed test binaries.
package testenv
...
const fakeAgentChildArgument = "--magicite-fake-agent-child"

func RunFakeAgent() {
	if len(os.Args) == 2 && os.Args[1] == fakeAgentChildArgument {
		runChild()
		return
	}
	if err := run(); err != nil {
...
func run() error {
	if err := Record(os.Getenv("MAGICITE_TRACE"), os.Args, fakeAgentWorkingDirectory()); err != nil {
	...
	scenario, err := AgentScenario(os.Getenv("MAGICITE_FAKE_AGENT_STORE"), plan, os.Getenv("MAGICITE_FAKE_AGENT_SCENARIO"))
	if err != nil {
...
func emit(sessionID, line string) error {
	...
	return RecordAgentEvent(os.Getenv("MAGICITE_FAKE_AGENT_EVENTS"), sessionID, line)
}
...
func hang(ctx context.Context) error {
	...
	child := exec.Command(os.Args[0], fakeAgentChildArgument)
	child.Stdout, child.Stderr = io.Discard, io.Discard
	if err := child.Start(); err != nil {
		return fmt.Errorf("start fake agent child: %w", err)
	}
	if err := RecordAgentChildPID(os.Getenv("MAGICITE_FAKE_AGENT_PIDS"), child.Process.Pid); err != nil {
...
func fakeAgentWorkingDirectory() string {
	dir, err := os.Getwd()
	if err != nil {
		return ""
	}
